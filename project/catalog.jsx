...
(function () {
  ...
  // Page through every row of /api/<table> matching opts (no fixed cap on
  // top of `hardCap`). Used by panels that need full-snapshot aggregates —
  // the truncated `limit=N` form silently drops rows beyond N, which made
  // the "Most-used producers" panel and "Created Records" tab undercount
  // for high-volume templates. hardCap is a sanity stop so a runaway
  // query (or a misconfigured table) can't burn unbounded memory.
  async function fetchAllRows(table, opts = {}, hardCap = 50_000) {
    ...
    while (offset < hardCap) {
      ...
      if (r.rows.length < pageSize) break;     // last page
      if (rows.length >= total) break;          // got everything
      offset += pageSize;
    }
    const capped = rows.length < (total || 0);
    return { rows, total: total || rows.length, missing: false, capped };
  }

  // ---- Item kind classification -----------------------------------------
  // Maps sys_class_name to a friendly category the UI can chip + filter by.
  // 'producer' covers any producer-shaped subclass — those don't create
  // RITMs (they spawn records in target tables) so the dashboard's RITM
  // overlay under-counts them, and the record view shows a "Created
  // records" tab instead of the usual variables-driven flow.
  const KIND_BY_CLASS = {
    'sc_cat_item':                    { kind: 'item',     label: 'Item' },
    'pc_software_cat_item':           { kind: 'software', label: 'Software' },
    'sc_cat_item_content':            { kind: 'content',  label: 'Content' },
    'sc_cat_item_producer':           { kind: 'producer', label: 'Record Producer' },
    'std_change_record_producer':     { kind: 'std_chg',  label: 'Std Change Template' },
    'sc_cat_item_composite_producer': { kind: 'composite', label: 'Composite Producer' },
  };
  function kindOf(it) {
    const cls = (it && it.sys_class_name && (it.sys_class_name.value || it.sys_class_name)) || 'sc_cat_item';
    return KIND_BY_CLASS[cls] || { kind: 'other', label: String(cls).replace(/_/g, ' ') };
  }
  const PRODUCER_KINDS = new Set(['producer', 'std_chg', 'composite']);

  // ---- Public catalog API ----------------------------------------------
  const C = {
    KIND_BY_CLASS, PRODUCER_KINDS, kindOf,
    ...
    // Every standard-change `change_request` in the snapshot, paged from
    // /api/change_request?type=standard. Cached on first call so the
    // dashboard doesn't re-page on every render. Live snapshot has ~2,174
    // standards out of ~14,974 total change_requests.
    fetchAllStandardChanges: () => fetchCached('all_std_changes', () =>
      fetchAllRows('change_request', {
        filters: { type: 'standard' },
        order_by: 'sys_updated_on', dir: 'desc',
      }),
      120_000,
    ),
    // For Standard Change templates: producer.name -> all proposal versions
    // -> all change_request rows that reference any of those proposal sys_ids.
    // Pages through every change_request — high-volume templates have 600+
    // rows, so the previous limit=200 silently truncated. Returns
    // { proposals, changes, total, capped }.
    fetchProducerCreatedRecords: async (producerName) => {
      if (!producerName) return { proposals: [], changes: [], total: 0 };
      const propRes = await fetchTable('std_change_proposal', {
        limit: 50, filters: { template_name: producerName }, order_by: 'sys_updated_on', dir: 'desc',
      });
      if (propRes.missing) return { proposals: [], changes: [], total: 0, missing: true };
      const proposals = propRes.rows;
      // Fan out one paged query per proposal sys_id (1-3 versions / template).
      const changeBuckets = await Promise.all(proposals.map(p =>
        fetchAllRows('change_request', {
          filters: { std_change_producer_version: p.sys_id },
          order_by: 'sys_updated_on', dir: 'desc',
        })
      ));
      const all = [];
      let total = 0, capped = false;
      for (let i = 0; i < changeBuckets.length; i++) {
        for (const r of changeBuckets[i].rows) all.push({ ...r, _proposal: proposals[i] });
        total += changeBuckets[i].total || 0;
        if (changeBuckets[i].capped) capped = true;
      }
      all.sort((a, b) => (b.sys_updated_on || '').localeCompare(a.sys_updated_on || ''));
      return { proposals, changes: all, total, capped };
    },
...
(function () {
  ...
  // Color for the kind chip. Std change templates get the change/blue, plain
  // producers get violet (distinct from item green), composites get amber.
  const KIND_CHIP = {
    item:      { bg: 'var(--bg-3)',         border: 'var(--border)',          fg: 'var(--fg-2)' },
    software:  { bg: 'var(--bg-3)',         border: 'var(--border)',          fg: 'var(--fg-2)' },
    content:   { bg: 'var(--bg-3)',         border: 'var(--border)',          fg: 'var(--fg-2)' },
    producer:  { bg: 'var(--c-violet-bg)',  border: 'var(--c-violet-border)', fg: 'var(--c-violet)' },
    std_chg:   { bg: 'var(--c-blue-bg)',    border: 'var(--c-blue-border)',   fg: 'var(--c-blue)' },
    composite: { bg: 'var(--c-amber-bg)',   border: 'var(--c-amber-border)',  fg: 'var(--c-amber)' },
    other:     { bg: 'var(--bg-3)',         border: 'var(--border)',          fg: 'var(--fg-2)' },
  };
  function KindChip({ it }) {
    const k = C.kindOf(it);
    const s = KIND_CHIP[k.kind] || KIND_CHIP.other;
    return <span style={{ ...chip, background: s.bg, borderColor: s.border, color: s.fg }}>{k.label}</span>;
  }
  // Strip the trailing " - N" version suffix from the std change producer
  // version display so we can group RITM-/change-counts by template name.
  // Display format on change_request.std_change_producer_version is, e.g.,
  // "BAU 091 - Renew SAP JAVA Standard SSL certificate - 2".
  function stripVersionSuffix(s) {
    return String(s || '').replace(/\s+-\s+\d+$/, '');
  }
  ...
  window.CatalogOverviewPage = function CatalogOverviewPage() {
    ...
    const [stdChanges, setStdChanges] = useState(null);  // recent Standard CRs
    ...
    useEffect(() => {
      ...
      // Standard change templates create change_requests, not RITMs. Page
      // through every one in the snapshot (not just the most recent N) so
      // the "Most-used record producers" counts reflect actual snapshot
      // history, not whatever happened in the last few months.
      C.fetchAllStandardChanges()
        .then(r => { if (!cancel) setStdChanges(r.rows); })
        .catch(() => { if (!cancel) setStdChanges([]); });
      return () => { cancel = true; };
    }, []);

    // Most-used record producers, ranked by # of change_requests created
    // from the template. Display values on change_request.std_change_producer_version
    // include " - N" version suffix; strip it to get the producer name.
    const topProducers = useMemo(() => {
      if (!items || !stdChanges) return null;
      const itemsByName = new Map();
      for (const it of items) itemsByName.set(String(it.name || ''), it);
      const counts = new Map();
      for (const cr of stdChanges) {
        const tmpl = stripVersionSuffix(dv(cr, 'std_change_producer_version'));
        if (!tmpl) continue;
        if (!counts.has(tmpl)) counts.set(tmpl, { name: tmpl, total: 0, item: itemsByName.get(tmpl) || null });
        counts.get(tmpl).total += 1;
      }
      return [...counts.values()].sort((a, b) => b.total - a.total).slice(0, 12);
    }, [items, stdChanges]);
    const stdChgPctOfWindow = stdChanges ? Math.round((stdChanges.length / Math.max(1, stdChanges.length)) * 100) : 0;
    ...
    return (
      <div style={{ padding: '32px 32px 60px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        {/* Most-used record producers (Standard Change templates).
            These create change_request rows, not RITMs, so they're invisible
            to the "Most-used items" panel above — surface them separately. */}
        <div style={{ marginTop: 28 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 10 }}>
            <h2 style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--fg-3)', margin: 0 }}>
              Most-used record producers
              <span className="mono" style={{ color: 'var(--fg-4)', marginLeft: 8 }}>
                from {stdChanges == null ? '…' : stdChanges.length.toLocaleString()} standard changes
              </span>
            </h2>
            <span style={{ fontSize: 11, color: 'var(--fg-4)' }}>
              counts join change_request → std_change_proposal → std_change_record_producer
            </span>
          </div>
          <div style={{ background: 'var(--bg-elev)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
            {topProducers == null ? <Loading /> : topProducers.length === 0 ? (
              <div style={{ padding: 24, color: 'var(--fg-4)', fontSize: 12.5, textAlign: 'center' }}>
                No standard changes in the snapshot reference catalog producers.
              </div>
            ) : (
              <table className="dt" style={{ width: '100%' }}>
                <thead><tr>
                  <th>Standard Change template</th>
                  <th style={{ width: 120 }} className="num">Changes created</th>
                  <th style={{ width: 200 }}>Catalog item</th>
                </tr></thead>
                <tbody>
                  {topProducers.map(p => (
                    <tr key={p.name}
                        onClick={() => p.item && window.navigate(`/catalog-items/${p.item.sys_id}`)}
                        style={{ cursor: p.item ? 'pointer' : 'default' }}>
                      <td>
                        <strong style={{ fontWeight: 500 }}>{p.name}</strong>
                      </td>
                      <td className="num mono">{p.total.toLocaleString()}</td>
                      <td>
                        {p.item
                          ? <span style={{ ...chip, background: KIND_CHIP.std_chg.bg, borderColor: KIND_CHIP.std_chg.border, color: KIND_CHIP.std_chg.fg }}>Open template →</span>
                          : <span style={{ color: 'var(--fg-4)', fontSize: 11.5, fontStyle: 'italic' }}>(no matching sc_cat_item)</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
  ...
  window.CatalogItemListPage = function CatalogItemListPage() {
    ...
    const [kind, setKind] = useState('');  // empty = all kinds
    ...
    // Counts per kind (drives the dropdown labels so you can see
    // how many of each type exist without filtering first).
    ...
    const filtered = useMemo(() => {
      ...
      return items.filter(i => {
        ...
        if (kind && C.kindOf(i).kind !== kind) return false;
    ...
    }, [items, q, catalog, category, kind, activeOnly]);
        ...
        <div className="page-header">
          ...
          <div className="toolbar">
            <select value={kind} onChange={e => setKind(e.target.value)}
              ...
              <option value="">All types ({items ? items.length : '…'})</option>
              {kindCounts.map(k => (
                <option key={k.kind} value={k.kind}>{k.label} ({k.count})</option>
              ))}
            </select>
  ...
  window.CatalogItemRecordPage = function CatalogItemRecordPage({ sys_id, showRaw }) {
    ...
    const [created, setCreated] = useState(null);  // producer-created records

    useEffect(() => {
      let cancel = false;
      setRec(null); setRelated({}); setUsage(null); setCreated(null);
      C.fetchItem(sys_id).then(r => {
        ...
        // For Standard Change templates, follow producer → proposal → change.
        // For other producer subtypes, target table varies and isn't always
        // recoverable from the envelope, so we only run the join for std_chg.
        const k = C.kindOf(r);
        if (k.kind === 'std_chg') {
          C.fetchProducerCreatedRecords(flat(r.name))
            .then(cr => { if (!cancel) setCreated(cr); })
            .catch(() => { if (!cancel) setCreated({ proposals: [], changes: [], total: 0, missing: true }); });
        } else if (C.PRODUCER_KINDS.has(k.kind)) {
          // Other producers: surface a "Created records" tab but explain why
          // we can't enumerate the spawned records.
          setCreated({ proposals: [], changes: [], total: 0, unknownTarget: true });
        }
      }).catch(e => {
        if (cancel) return;
    ...
    const itemKind = C.kindOf(rec);
    const isProducer = C.PRODUCER_KINDS.has(itemKind.kind);
    const tabs = [
      // For producer items, lead with "Created records" since variables
      // typically aren't relevant to record producers (the producer's
      // template defines the spawned record's fields directly).
      ...(isProducer ? [{
        id: 'created',
        label: itemKind.kind === 'std_chg' ? 'Changes Created' : 'Created Records',
        r: created == null
          ? { loading: true, total: 0 }
          : { total: created.total ?? created.changes.length, missing: created.missing, unknownTarget: created.unknownTarget },
      }] : []),
    ...
    // When the active tab disappears (e.g. user nav-changes from a producer
    // item to a regular item and back), default to the first tab so we
    // don't render nothing.
    const visibleIds = new Set(tabs.map(t => t.id));
    const activeTab = visibleIds.has(tab) ? tab : tabs[0].id;
        ...
        <div className="left">
          ...
          <div className="section">
            ...
            <div className="fields" style={{ display: 'grid', gridTemplateColumns: '180px 1fr', rowGap: 8, columnGap: 16 }}>
              ...
              {flat(rec.flow_designer_flow) && (
                <Field label="Flow">
                  <a style={{ color: 'var(--accent-fg)', cursor: 'pointer' }}
                     onClick={() => window.navigate(window.recordUrl('flow_inventory', flat(rec.flow_designer_flow)))}>
                    {dv(rec, 'flow_designer_flow') || flat(rec.flow_designer_flow)}
                  </a>
                </Field>
              )}
          ...
          <div className="section">
            <CatalogTabs tabs={tabs} active={activeTab} onChange={setTab} />
            <div style={{ paddingTop: 8 }}>
              {activeTab === 'created'    && <CreatedRecordsTab created={created} kind={itemKind.kind} />}
              {activeTab === 'variables'  && <VariablesTab r={r('variables')} />}
              {activeTab === 'vsets'      && <VariableSetsTab r={r('vsets')} />}
              {activeTab === 'policies'   && <UIPoliciesTab r={r('policies')} actions={r('actions')} />}
              {activeTab === 'scripts'    && <ClientScriptsTab r={r('scripts')} />}
              {activeTab === 'available'  && <UserCriteriaTab r={r('available')} kind="allow" />}
              {activeTab === 'notavail'   && <UserCriteriaTab r={r('notavail')} kind="deny" />}
              {activeTab === 'categories' && <CategoriesTab name={cat} sys_id={flat(rec.category)} />}
              {activeTab === 'catalogs'   && <CatalogsTab name={cata} sys_id={flat(rec.sc_catalogs)} />}
              {activeTab === 'lookups'    && <NotInSnapshot table="sc_cat_item_data_lookup_definitions" />}
              {activeTab === 'articles'   && <NotInSnapshot table="m2m_kb_to_sc_cat_item" />}
              {activeTab === 'relitems'   && <NotInSnapshot table="sc_cat_item_related_items" />}
              {activeTab === 'topics'     && <NotInSnapshot table="topic_item" />}
            </div>
          </div>
  ...
  // ---- Created Records (producer-spawned) ------------------------------
  // For std change templates: shows the change_request rows that came out
  // of the template (joined via std_change_proposal). For other producer
  // subtypes the target table isn't recoverable from the envelope, so we
  // explain the mapping and link to ServiceNow's std change usage shape.
  function CreatedRecordsTab({ created, kind }) {
    if (created == null) return <Loading label="Joining change_request → std_change_proposal…" />;
    if (created.unknownTarget) {
      return (
        <Empty
          icon="info"
          text="This producer's target table isn't recorded in the envelope."
          hint="Generic Record Producers can spawn into incident, change_request, problem, rm_defect, etc. The target is set by a server-side script on order, so the spawned record doesn't carry a back-reference to the producer. To enumerate created records, scan the target table's source / origin field per producer."
        />
      );
    }
    if (created.missing) return <NotInSnapshot table="std_change_proposal" />;
    if (!created.changes.length) return <Empty text="No change_requests have been created from this template (yet)." />;

    const showProposals = (created.proposals || []).length > 1;
    return (
      <div>
        {showProposals && (
          <div style={{ marginBottom: 10, padding: '8px 12px', background: 'var(--accent-bg)', border: '1px solid var(--accent-border)', borderRadius: 6, fontSize: 12, color: 'var(--accent-fg)' }}>
            This template has {created.proposals.length} versions.{' '}
            Counts below combine all of them.
          </div>
        )}
        <table className="dt">
          <thead><tr>
            ...
            <th style={{ width: 130 }}>Assignment group</th>
            <th style={{ width: 110 }} className="num">Updated</th>
          </tr></thead>
          <tbody>
            {created.changes.slice(0, 100).map(cr => {
              const stDec = window.decodeChoice('change_request', 'state', flat(cr.state));
              const stCls = window.stateChipClass('change_request', flat(cr.state));
              return (
                <tr key={cr.sys_id} onClick={() => window.navigate(window.recordUrl('change_request', cr.sys_id))}>
                  <td className="num">{flat(cr.number)}</td>
                  <td><span className="truncate">{flat(cr.short_description) || '—'}</span></td>
                  <td>{flat(cr.state)
                    ? <span className={`chip ${stCls}`}>{stDec.label || flat(cr.state)}</span>
                    : <span className="muted">—</span>}</td>
                  <td className="muted" style={{ fontSize: 11.5 }}>{dv(cr, 'assignment_group') || '—'}</td>
                  <td className="num muted" title={flat(cr.sys_updated_on)}>{window.fmtRelative(flat(cr.sys_updated_on))}</td>
                </tr>
              );
            })}
          </tbody>
        </table>
        {created.changes.length > 100 && (
          <div style={{ padding: '10px 0', textAlign: 'center', color: 'var(--fg-4)', fontSize: 11.5 }}>
            Showing first 100 of {created.changes.length.toLocaleString()} loaded.
            {created.total > created.changes.length && (
              <> Snapshot total: {created.total.toLocaleString()}.</>
            )}
            {created.capped && (
              <> Hit the page-through cap; some older rows may not be loaded.</>
            )}
          </div>
        )}
      </div>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--fg-2)' }}>
                ...
                {members.map(([label, v]) => (
                  <div key={label} style={{ marginTop: 2 }}>
                    <span style={{ color: 'var(--fg-3)' }}>{label}: </span>{v}
                  </div>
                ))}
              </div>
            )}
