...
(function () {
  ...
  const C = {
    ...
    // Returns { proposals: [...], changes: [...], total: N }.
    fetchProducerCreatedRecords: async (producerName) => {
      ...
      // Fan out one query per proposal sys_id (typically 1-3 versions per template).
      const changeBuckets = await Promise.all(proposals.map(p =>
        fetchTable('change_request', {
          limit: 200,
      ...
      for (let i = 0; i < changeBuckets.length; i++) {
        for (const r of changeBuckets[i].rows) {
          all.push({ ...r, _proposal: proposals[i] });
        }
      }
      ...
      return {
        proposals,
        changes: all,
        total: changeBuckets.reduce((s, b) => s + (b.total || 0), 0),
      };
    },
...
(function () {
  ...
  window.CatalogOverviewPage = function CatalogOverviewPage() {
    ...
    useEffect(() => {
      ...
      // Standard change templates create change_requests, not RITMs. Pull
      // the standard ones to compute "Most-used record producers".
      data.fetchTaskList('change_request', {
        limit: 5000, filters: { type: 'standard' },
        order_by: 'sys_updated_on', dir: 'desc',
      })
      ...
      <div>
        ...
        {created.changes.length > 100 && (
          <div style={{ padding: '10px 0', textAlign: 'center', color: 'var(--fg-4)', fontSize: 11.5 }}>
            Showing first 100 of {created.changes.length}. Total in snapshot: {created.total.toLocaleString()}.
          </div>
        )}
      </div>
    );
