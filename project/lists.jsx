...
function KBListPage() {
  ...
  React.useEffect(() => {
    let cancelled = false;
    setLoading(true);
    data.fetchTaskList('kb_knowledge', { limit: PAGE_SIZE, offset: page * PAGE_SIZE, order_by: 'number', dir: 'desc' })
    ...
    <div>
      ...
      <table className="dt">
        <thead><tr>
          ...
          <th>Knowledge base</th><th style={{ width: 130 }}>State</th><th style={{ width: 180 }}>Author</th>
        </tr></thead>
        <tbody>
          {loading && resp.rows == null && (
            <tr><td colSpan={5} style={{ padding: 60, color: 'var(--fg-4)', textAlign: 'center' }}>
          ...
          {!loading && resp.rows && resp.rows.length === 0 && (
            <tr><td colSpan={5} style={{ padding: 40, color: 'var(--fg-4)', textAlign: 'center' }}>No articles.</td></tr>
          )}
          {(resp.rows || []).map(a => (
            <tr key={a.sys_id} onClick={() => window.navigate(`/knowledge/${a.sys_id}`)}>
              ...
              <td className="muted">{a.__display_kb_knowledge_base || a.kb_knowledge_base || '—'}</td>
              <td>{a.workflow_state ? <span className="chip" style={{ fontSize: 10.5 }}>{a.__display_workflow_state || a.workflow_state}</span> : <span className="muted">—</span>}</td>
