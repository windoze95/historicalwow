  ...
  function getIncludeIndex() {
    ...
    _includeIndex = fetchAllRows('sys_script_include', {
      ...
      const list = r.rows.map(row => {
        ...
        // Drop blocklisted bare names; keep the namespaced api_name
        // form so a `global.JSON.parse(...)` call (when explicitly
        // qualified) can still resolve, while bare `JSON.parse(...)`
        // — which is the dominant case — doesn't drag the override
        // include into the cascade.
        for (const n of [...names]) {
          if (BUILTIN_NAME_BLOCKLIST.has(n)) names.delete(n);
        }
        return {
