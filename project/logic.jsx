...
---`);
  ...
  function FlowStepModal({ entry, onClose }) {
    ...
    const meta = [['order', sv(item.order)], ['ui_id', sv(item.ui_id)], ['parent_ui_id', sv(item.parent_ui_id)]].filter(m => m[1] != null && m[1] !== '');
    return (
  ...
  // Assemble EVERYTHING known about a flow into one copy-pasteable markdown
  // document — enough to reconstruct / reverse-engineer it (e.g. paste into an
  // LLM). Pulls the full, untruncated decoded config for every step.
  function flowContextMarkdown(rec) {
    const g = (k) => flat(rec[k]) || '';
    const out = [];
    out.push(`# Flow: ${g('name')}`, '');
    out.push(`- internal_name: ${g('internal_name')}`);
    out.push(`- sys_id: ${g('sys_id')}`);
    out.push(`- type: ${g('flow_type')}`);
    out.push(`- run_as: ${g('run_as')}`);
    out.push(`- scope: ${g('scope')}`);
    out.push(`- status: ${g('status')}`);
    out.push(`- active: ${isTrue(g('active'))}`);
    if (g('catalog_item')) out.push(`- catalog_item: ${g('catalog_item')}`);
    if (g('area')) out.push(`- area: ${g('area')}`);
    if (g('pattern')) out.push(`- pattern: ${g('pattern')}`);
    out.push('');
    const trig = rec.trigger;
    if (trig && (trig.type || trig.table || trig.condition)) {
      out.push('## Trigger');
      out.push(`- type: ${trig.type || ''}`);
      if (trig.table) out.push(`- table: ${trig.table}`);
      if (trig.condition) out.push(`- condition: ${trig.condition}`);
      out.push('');
    }
    if (g('narrative')) out.push('## What it does (curated narrative)', g('narrative'), '');
    if (g('known_issues')) out.push('## Known issues', g('known_issues'), '');
    const lc = Array.isArray(rec.label_cache_steps) ? rec.label_cache_steps : [];
    if (lc.length) { out.push('## Steps (as ServiceNow renders them)'); lc.forEach(s => out.push(`- ${s}`)); out.push(''); }
    const steps = Array.isArray(rec.steps) ? rec.steps : [];
    if (steps.length) {
      out.push('## Step details (actions, in order)');
      steps.forEach((s, i) => {
        out.push('', `### [order ${s.order || i + 1}] ${s.action_type || 'Step'}${s.parent_ui_id ? ' (nested)' : ''}`);
        if (s.display_text && s.display_text !== s.action_type) out.push(s.display_text);
        out.push(`- ui_id: ${s.ui_id || ''}${s.parent_ui_id ? `   parent_ui_id: ${s.parent_ui_id}` : ''}`);
        const cfg = s.config || {};
        const keys = Object.keys(cfg);
        if (keys.length) {
          out.push('- inputs:');
          keys.forEach(k => {
            ...
            const disp = (v.display != null && v.display !== val) ? `   (display: ${v.display})` : '';
            const ref = v.ref ? `   [ref -> ${v.ref}]` : '';
            out.push(`    - ${k}: ${val}${disp}${ref}`);
          });
        }
      });
      out.push('');
    }
    const logic = Array.isArray(rec.logic_blocks) ? rec.logic_blocks : [];
    if (logic.length) {
      out.push('## Flow logic (control blocks)');
      logic.forEach(b => out.push(`- [order ${b.order || ''}] ${b.kind || ''}${b.display_text ? ` - ${b.display_text}` : ''} (ui_id ${b.ui_id || ''})`));
      out.push('');
    }
    const subs = Array.isArray(rec.subflows) ? rec.subflows : [];
    if (subs.length) { out.push('## Subflows called'); subs.forEach(s => out.push(`- ${s.name}${s.sys_id ? ` (${s.sys_id})` : ''}`)); out.push(''); }
    const eps = Array.isArray(rec.integration_endpoints) ? rec.integration_endpoints : [];
    if (eps.length) { out.push('## Integration endpoints'); eps.forEach(e => out.push(`- ${e.kind}: ${e.id || e.name || ''}${e.step ? ` (step: ${e.step})` : ''}`)); out.push(''); }
    const st = rec.stats || {};
    if (st.run_count) {
      out.push('## Execution history');
      out.push(`- runs: ${st.run_count}, errored: ${st.error_count}, completed: ${st.complete_count}`);
      if (st.first_run) out.push(`- first run: ${st.first_run}`);
      if (st.last_run) out.push(`- last run: ${st.last_run}`);
      out.push('');
    }
    if (g('notables')) out.push('## Notables', g('notables'), '');
    out.push(`_Source: ServiceNow flow archive (sys_hub_flow ${g('sys_id')}). Raw step/trigger/logic records are mirrored in sys_hub_action_instance_v2 / sys_hub_trigger_instance / sys_hub_flow_logic (filter by flow=${g('sys_id')})._`);
    return out.join('\n');
  }

  // Clipboard write that also works over plain HTTP (no secure-context
  // clipboard API): falls back to a temporary textarea + execCommand.
  function flowCopyText(text) {
    try {
  ...
  function flowDownloadText(filename, text) {
  ...
  window.FlowInventoryRecordPage = function FlowInventoryRecordPage({ sys_id }) {
    ...
    const rawByUid = new Map();   // ui_id -> full record; Map (not {}) since keys come from data
    if (haveRaw) [...(rawData.actions || []), ...(rawData.logic || [])].forEach(r => { const u = r && r.ui_id; if (u) rawByUid.set(String(u), r); });
    const openStep = (fi) => setStepModal({
      item: fi.item, number: fi.number, kind: fi.kind === 'logic' ? 'Control flow' : 'Action',
      raw: rawByUid.get(String(fi.item.ui_id || '')) || null,
    });
